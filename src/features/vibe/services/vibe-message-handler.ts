 ...
 */
export function extractFilesFromCodeBlocks(
...
export function detectProjectStructure(files: ExtractedFile[]): {
  ...
  if (hasReactFiles) {
    return {
      type: 'react',
      entryPoint:
        paths.find((p) => p.includes('index.html')) ||
        paths.find((p) => p.includes('App')) ||
        null,
  ...
  const hasServerFiles = paths.some(
    (p) =>
      p.includes('server.') || p.includes('index.js') || p.includes('app.js')
  );
  ...
  if (hasServerFiles) {
    return {
      type: 'node',
      entryPoint:
        paths.find((p) => p.includes('server.') || p.includes('index.js')) ||
        null,
