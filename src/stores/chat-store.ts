  ...
  useChatStore((state) => {
    ...
    if (state.filterTags.length > 0) {
      filtered = filtered.filter((conv) =>
        state.filterTags.some((tag) => conv.metadata.tags.includes(tag))
      );
    }
...
export const useChatModels = () => useChatStore((state) => state.availableModels);
export const useSelectedModel = () => useChatStore((state) => state.selectedModel);
export const useChatLoading = () => useChatStore((state) => state.isLoading || state.isStreamingResponse);
