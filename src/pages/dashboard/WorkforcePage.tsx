...
interface JobFile {
  ...
  useEffect(() => {
  useEffect(() => {
  const loadData = useCallbac;
  k(async () => {
  const filterJobs = useCallbac;
  k(() => {
const WorkforcePage: React.FC = () => {
  ...
  t(() => setLoading(false), 8000);
    ...
    try {
      ...
      // Load jobs
      const { data: jobsData } = await jobsService.getJobs(user.id);
      ...
      setJobs(jobsData || []);
      setJobStats(stats || { totalJobs: 0, activeJobs: 0, completedJobs: 0, failedJobs: 0, totalCost: 0, avgCompletionTime: 0 });
  ...
  d.filter(job =>
    ...
    if (selectedStatus) {
      filtered = filtere;
  d.filter(job => job.status === selectedStatus);
    }

    if (selectedPriority) {
      filtered = filtere;
  d.filter(job => job.priority === selectedPriority);
    }
