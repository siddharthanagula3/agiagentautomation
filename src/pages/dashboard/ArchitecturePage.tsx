...
interface Component {
  id: string;
  name: string;
  type: 'service' | 'database' | 'api' | 'frontend' | 'backend' | 'infrastructure' | 'security';
...
const ArchitecturePage: React.FC = () => {
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      ...
      case 'infrastructure':
        return <Building className="h-4 w-4 text-yellow-500" />;
      case 'security':
  ...
  const architectureStats = {
    ...
    avgUptime: components.length > 0 ? components.reduce((sum, c) => sum + c.metrics.uptime, 0) / components.length : 0
  };
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Monitor and manage your system architecture and component health.
          </p>
        </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-foreground">{architectureStats.total}</p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-green-600">{architectureStats.healthy}</p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-yellow-600">{architectureStats.degraded}</p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-blue-600">{architectureStats.avgUptime.toFixed(1)}%</p>
              </div>
                ...
                <div className="flex-1">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm mb-4">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="font-medium">{component.metrics.throughput}/s</span>
                    </div>
                    <div className="flex items-center space-x-2">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="font-medium">{component.metrics.cpuUsage}%</span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <BarChart3 className="h-4 w-4 text-muted-foreground" />
                      <span className="text-muted-foreground">Memory:</span>
                      <span className="font-medium">{component.metrics.memoryUsage}%</span>
                    </div>
                  </div>
                  ...
                  {/* Dependents */}
                  <div className="mb-4">
                    <h4 className="text-sm font-medium text-foreground mb-2">Dependents</h4>
                    <div className="flex flex-wrap gap-2">
                      {component.dependents.map((dep, index) => (
                        <Badge key={index} variant="outline" className="text-xs">
                          {dep}
                        </Badge>
                      ))}
                      {component.dependents.length === 0 && (
                        <span className="text-xs text-muted-foreground">No dependents</span>
                      )}
                    </div>
                  </div>
