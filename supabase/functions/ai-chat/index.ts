...
serve(async (req) => {
  ...
  try {
    ...
    // Parse request body
    const { messages, provider = 'gemini', model, sessionId } = await req.json()
...
async function callAnthropic(messages: any[], model = 'claude-3-5-sonnet-20241022'): Promise<string> {
...
async function callGemini(messages: any[], model = 'gemini-2.0-flash'): Promise<string> {
