...
export function TerminalView() {
  ...
  // Mock terminal history for demonstration
  const mockHistory = terminalState.history.length > 0 ? terminalState.history : [
    {
      id: '1',
      command: 'npm install',
      output: 'added 1247 packages in 12.3s\n\n128 packages are looking for funding',
        ...
        <div className="grid grid-cols-3 gap-4 text-center text-xs">
          <div>
            <div className="text-lg font-bold text-green-600">
              {mockHistory.filter((c) => c.status === 'completed' && c.exitCode === 0).length}
            </div>
            <div className="text-muted-foreground">Success</div>
          </div>
          <div>
            <div className="text-lg font-bold text-blue-600">
              {mockHistory.filter((c) => c.status === 'running').length}
            </div>
            <div className="text-muted-foreground">Running</div>
          </div>
          <div>
            <div className="text-lg font-bold text-red-600">
              {mockHistory.filter((c) => c.status === 'failed').length}
            </div>
