...
async function testForgotPasswordFlow(browser) {
  ...
  try {
    ...
    const pageContent = await page.content();
    if (pageContent.includes('Check your email') || pageContent.includes('sent a password reset link')) {
      success('Forgot password form submitted successfully');
      return { success: true, page };
    } else {
      error('No success message found after submission');
      return { success: false, page };
    }
  } catch (err) {
...
async function testLogoutFlow(browser) {
  ...
  try {
    ...
    const userMenuButton = await page.$('button[aria-label="User menu"]') ||
