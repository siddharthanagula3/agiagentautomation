...
export async function listPurchasedEmployees(userId?: string | null): Promise<PurchasedEmployeeRecord[]> {
  ...
  try {
    ...
    if (error) {
      ...
      if (error.message?.includes('relation "purchased_employees" does not exist') || 
  ...
  try {
    ...
    if (error) {
      ...
      if (error.message?.includes('relation "purchased_employees" does not exist') || 
          error.message?.includes('does not exist') ||
          error.code === '42P01') {
        throw new Error('DATABASE_SETUP_REQUIRED: The purchased_employees table needs to be created. Please run the database setup script in Supabase.');
      }
