...
async function listWebhooks() {
  ...
  webhooks.data.forEach((webhook, index) => {
    ...
    if (
      webhook.enabled_events.length > 0 &&
      webhook.enabled_events.length <= 10
    ) {
...
async function createProductionWebhook(url: string) {
  ...
  try {
    ...
    if (webhook.secret) {
      ...
      console.log(
        '⚠️  IMPORTANT: Set this as STRIPE_WEBHOOK_SECRET in Netlify!\n'
      );
    } else {
      console.log('🔑 To get the signing secret:');
      console.log(
        `   1. Go to: https://dashboard.stripe.com/webhooks/${webhook.id}`
      );
...
async function main() {
  ...
  console.log(
    '⚠️  This will DELETE all sandbox/test webhooks and create production webhook\n'
  );
  console.log('='.repeat(70) + '\n');
  ...
  if (!productionUrl) {
    ...
    console.log(
      '   Example: $env:PRODUCTION_URL="https://your-site.netlify.app"\n'
    );
    process.exit(1);
  }
  ...
  if (existingProd) {
    ...
    if (missingEvents.length > 0) {
      ...
      console.log(
        '   Update in Stripe Dashboard to include all required events.\n'
      );
    } else {
    ...
    console.log('🔑 To get the signing secret:');
    console.log(
      `   https://dashboard.stripe.com/webhooks/${existingProd.id}\n`
    );
  } else {
  ...
  console.log(
    '   3. Ensure you are using LIVE keys (sk_live_...) for production payments\n'
  );
  console.log(
    '⚠️  IMPORTANT: Make sure you are using LIVE Stripe keys for real payments!'
  );
