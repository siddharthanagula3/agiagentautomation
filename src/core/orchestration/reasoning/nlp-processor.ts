...
export class NLPProcessor {
  ...
  private async extractRequirements(
    ...
    if (requirements.length === 0) {
      requirements.push(
        `Complete task related to: ${keywords.slice(0, 5).join(', ')}`
      );
    }
  ...
  getSuggestions(input: string): string[] {
    ...
    if (input.length < 10) {
      suggestions.push(
        'Try providing more details about what you want to accomplish'
      );
    }
    ...
    if (!hasVerb) {
      suggestions.push(
        'Start with an action verb (create, build, modify, analyze, etc.)'
      );
    }
