    ...
    if (authenticatedUserId) {
      ...
      if (!balanceCheck.hasBalance) {
        return createInsufficientBalanceResponse(
          { required: balanceCheck.balance ?? 0, available: balanceCheck.balance || 0 },
          corsHeaders
        );
      }
    }
    ...
    if (data.usage) {
      ...
      if (verifiedUserId) {
        ...
        const deductResult = await deductCredits(
          ...
          actualCostCents,
          `OpenAI ${model} usage`,
          { provider: 'openai', model, inputTokens: tokenUsage.inputTokens, outputTokens: tokenUsage.outputTokens },
          `openai-${verifiedUserId}-${Date.now()}`
        );

        if (!deductResult.success) {
          console.error('[OpenAI Proxy] Credit deduction failed:', deductResult.error);
