...
export const testSupabaseConnection = async () => {
  ...
  try {
    ...
    console.log('[TestSupabase] Auth check:', {
      user: user?.id,
      error: authError,
    });
    ...
    if (queryError) {
      ...
      return {
        ...
        details: queryError,
        suggestion:
          'Check if the purchased_employees table exists and has proper RLS policies',
      };
    }
    ...
    console.log('[TestSupabase] Insert test:', {
      data: insertData,
      error: insertError,
    });
    ...
    if (insertError) {
      ...
      if (insertError.code === '23505') {
        console.log(
          '[TestSupabase] Unique constraint violation (expected for duplicate test)'
        );
    ...
    console.log('[TestSupabase] Duplicate constraint test:', {
    ...
    if (duplicateError && duplicateError.code === '23505') {
      ...
      console.warn(
        '[TestSupabase] ⚠️ Unexpected error on duplicate test:',
        duplicateError
      );
    } else {
      console.warn(
        '[TestSupabase] ⚠️ Unique constraint may not be working - duplicate was inserted'
      );
    ...
    return {
      success: true,
      message:
        'Supabase connection and purchased_employees table working correctly',
      userId: user.id,
      tests: [
        'Authentication',
        'Table query',
...
if (typeof window !== 'undefined') {
  (
    window as { testSupabaseConnection?: typeof testSupabaseConnection }
