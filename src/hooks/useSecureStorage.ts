...
export const useSecureStorage = <T>(
  ...
  const { encrypt = true, ttl, storage = 'localStorage' } = options;
  ...
  const [storedValue, setStoredValue] = useState<T>(() => {
    try {
      ...
      let parsedItem: StoredItem<T> | EncryptedStoredItem | T =
        JSON.parse(item);
...
export const useTemporaryStorage = <T>(
  key: string,
  defaultValue: T,
  ttlMinutes = 60
) => {
