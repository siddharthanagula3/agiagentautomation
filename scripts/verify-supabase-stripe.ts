...
async function verifySupabase() {
  ...
  if (!supabaseUrl || !supabaseKey) {
    ...
    log('   Required: SUPABASE_SERVICE_ROLE_KEY or VITE_SUPABASE_ANON_KEY', 'yellow');
    return false;
  }
  ...
  try {
    ...
    if (!plans || plans.length === 0) {
      ...
      log('   Run migration: supabase/migrations/20250112000001_update_pro_pricing.sql', 'yellow');
    } else {
      ...
      if (proPlan.price_monthly === 29 && proPlan.price_yearly === 299.88) {
        ...
        log(`   Actual: $${proPlan.price_monthly}/month, $${proPlan.price_yearly}/year`, 'yellow');
        log('   Run migration: supabase/migrations/20250112000001_update_pro_pricing.sql', 'yellow');
      }
    }
    ...
    const requiredTables = ['user_shortcuts', 'public_artifacts', 'token_transactions'];
    ...
    if (userColumnsError && userColumnsError.code === 'PGRST202') {
      ...
      log('   ❌ token_balance or subscription_start_date columns missing', 'red');
      log('   Run migrations: 20250111000003_add_token_system.sql and 20250111000004_add_subscription_start_date.sql', 'yellow');
    } else if (userColumnsError) {
      log(`   ⚠️  Error checking columns: ${userColumnsError.message}`, 'yellow');
    } else {
...
async function verifyStripe() {
  ...
  if (isLive) {
    ...
    log('   Switch to LIVE keys (sk_live_...) for production payments', 'yellow');
  } else {
  ...
  try {
    ...
    log(`✅ Charges Enabled: ${account.charges_enabled ? 'Yes' : 'No'}`, account.charges_enabled ? 'green' : 'yellow');
    log(`✅ Payouts Enabled: ${account.payouts_enabled ? 'Yes' : 'No'}`, account.payouts_enabled ? 'green' : 'yellow');
    ...
    if (webhooks.data.length === 0) {
      ...
      const productionWebhooks = webhooks.data.filter(w => 
      ...
      if (productionWebhooks.length > 0) {
        ...
        log('   Create webhook for: https://agiagentautomation.com/.netlify/functions/stripe-webhook', 'yellow');
      }

      // Check for sandbox/test webhooks
      const sandboxWebhooks = webhooks.data.filter(w => 
        w.url.includes('localhost') || 
        w.url.includes('test') || 
      ...
      if (sandboxWebhooks.length > 0) {
        log('   ⚠️  Found sandbox/test webhooks (should be removed):\n', 'yellow');
        sandboxWebhooks.forEach((webhook, index) => {
          log(`   ${index + 1}. ${webhook.url} (ID: ${webhook.id})`, 'yellow');
        });
        log('\n   Delete these in Stripe Dashboard: https://dashboard.stripe.com/webhooks', 'yellow');
      }
    }
