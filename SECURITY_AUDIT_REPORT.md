...
**Risks**:
- VITE_ prefixed environment variables are exposed to the browser
...
- Use environment variables without VITE_ prefix for server-side secrets
...
const isDemoMode = !supabaseUrl || !supabaseKey || supabaseUrl.includes('your-project-url');
```
...
```typescript
setTimeout(() => resolve({ user: null, error: 'Auth initialization timeout' }), 5000)
```
